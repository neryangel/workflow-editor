...
export function ExecutionHistoryPanel({ onReplay }: ExecutionHistoryPanelProps) {
    ...
    const handleReplay = (id: string) => {
        ...
        if (replay && onReplay) {
            onReplay(
                replay.nodes as unknown as Node[],
                replay.edges as unknown as Edge[],
                replay.variables
            );
            setIsOpen(false);
        }
    ...
    return (
        <>
          ...
          transition-all duration-200 shadow-lg`}
            ...
            {/* Panel */}
            {isOpen && (
                <div className="fixed left-0 top-0 bottom-0 w-96 bg-slate-900 border-r border-slate-700 z-40 flex flex-col">
                    {/* Header */}
                                    ...
                                    const status =
                                        e.target.value === 'all'
                                            ...
                                            : (e.target.value as
                                                  ...
                                                  | 'failed'
                                                  | 'cancelled');
                                    setFilter({ ...currentFilter, status });
                                }}
                                ...
                                >
                                    ...
                                    <div className="text-xs text-slate-400 space-y-1">
                                        <div>
                                            ...
                                            {execution.edges.length} connections
                                        </div>
                                        {execution.variables &&
                                            Object.keys(execution.variables).length > 0 && (
                                                <div>
                                                    {Object.keys(execution.variables).length}{' '}
                                                    variables
                                                </div>
                                            )}
                                        {execution.error && (
                                            <div
                                                className="text-red-400 mt-1 truncate"
