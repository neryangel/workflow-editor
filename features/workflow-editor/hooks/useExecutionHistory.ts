...
function loadInitialHistory(): ExecutionLog[] {
    ...
    if (stored) {
        try {
            ...
            const withDates = parsed.map((item: ExecutionLog & { timestamp: string }) => {
...
export function useExecutionHistory(): UseExecutionHistoryReturn {
    const [history, setHistory] = useState<ExecutionLog[]>(loadInitialHistory);
    ...
    const getStats = useCallback((): ExecutionStats => {
        ...
        const avgDuration =
            total > 0 ? history.reduce((sum, item) => sum + item.duration, 0) / total : 0;
    ...
    const exportHistory = useCallback(
        (ids?: string[]) => {
            const toExport = ids ? history.filter((item) => ids.includes(item.id)) : history;
