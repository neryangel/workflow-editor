...
test.describe("Workflow Execution", () => {
  ...
  test("should display output after execution", async ({ page }) => {
    ...
    const _outputArea = page
      .locator('[data-testid="output"], .output, [class*="output"]')
      .first();
  ...
  test("should show error state on failed execution", async ({ page }) => {
    ...
    // Find error display area if any (feature may not exist yet)
    const _errorDisplay = page.locator(
      '[class*="error"], .error-message, [data-testid="error"]',
    );
