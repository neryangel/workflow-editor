...
test.describe("Keyboard Shortcuts", () => {
  ...
  test("should handle Ctrl+Shift+Z for redo (alternative)", async ({
    page,
  }) => {
    await page.locator(".react-flow").click();
...
test.describe("Template Loading", () => {
  ...
  test("should have template options available", async ({ page }) => {
    ...
    const templateButton = page
      ...
      .first();
    const templateMenu = page
      .locator('[class*="template"], .templates')
      .first();

    const _hasTemplateButton = await templateButton
      .isVisible()
      .catch(() => false);
    const _hasTemplateMenu = await templateMenu.isVisible().catch(() => false);
